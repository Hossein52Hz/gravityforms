...
-->
    ...
    public static function Request( $confirmation, $form, $entry, $ajax ) {
        ...
        try {
            ...
            $Result = wp_remote_post('https://vandar.io/api/ipg/send', array(
                'method' => 'POST',
                'headers'  => array(
            ...
            if ( ! empty( $Result['status'] ) && $Result['status'] ) {
                ...
                $payment_URL = sprintf( 'https://vandar.io/ipg/%s', $Result['token'] );
            ...
            if ( empty( $_GET['no'] ) || $_GET['no'] != 'true' ) {
                ...
                try {
                    ...
                    $trans_id     = isset( $_GET['token'] ) ? sanitize_text_field( $_GET['token'] ) : '';
                        ...
                        $Result = wp_remote_post('https://vandar.io/api/ipg/verify', array(
                            'method' => 'POST',
                            'headers'  => array(
                                'Content-type: application/x-www-form-urlencoded',
                                'Accept: application/json'
